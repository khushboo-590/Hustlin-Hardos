                ...
                <div className="max-w-[561px] mx-auto flex justify-center items-center">
                    {navLinks.map((obj, index) => (
                        <a
                            key={index}
                            href={`#${obj.link}`}
                            className={`px-6 uppercase hover:bg-black hover:text-white transition-all duration-300 whitespace-nowrap max-md:text-sm max-sm:text-xs border-solid py-[7px] max-sm:py-1 max-sm:px-2 border-black ${index === 0 ? 'border-x-2' : 'border-r-2'}`} >
                            {obj.title}
                        </a>
                    ))}
                </div>
